...
DECLARE @cust NVARCHAR(MAX)
  ...
  SELECT @cust = isnull (@cust+',','') +  QUOTENAME (c.cus)
    FROM (SELECT c.CustomerName AS cus
    FROM Sales.Customers c ) AS c

  SET @sql = N'SELECT
    d,'+@cust+'
  FROM (SELECT
      c.CustomerName AS cus
     ,COUNT(*) AS cnt
  ...
  ,''01.'' + CAST(DATEPART(MONTH,i.InvoiceDate) AS NVARCHAR) + ''.'' + CAST(DATEPART(year,i.InvoiceDate) AS NVARCHAR) AS d
    ...
    GROUP BY c.CustomerName,i.InvoiceDate) t
  PIVOT (
  SUM(cnt)
  FOR cus IN (' +@cust  +')
  ) p
